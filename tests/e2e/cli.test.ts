/**
 * Comprehensive E2E tests for the Kova CLI.
 *
 * Tests the CLI programmatically via the `run()` function from `@usekova/cli`,
...
import { buildCovenant, serializeCovenant } from '@usekova/core';
...
function makeTmpDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'kova-cli-test-'));
}
...
describe('CLI help and version', () => {
  it('run(["help"]) returns exitCode 0 and stdout lists all commands', async () => {
    ...
    expect(plain).toContain('Kova CLI');
...
describe('CLI init', () => {
  ...
  it('run(["init"], tmpDir) creates a config file in the temp directory', async () => {
    ...
    expect(r.exitCode).toBe(0);
    expect(r.stderr).toBe('');
    const configPath = path.join(tmpDir, 'kova.config.json');
  ...
  it('running init twice overwrites gracefully without error', async () => {
    ...
    const config1 = JSON.parse(
      fs.readFileSync(path.join(tmpDir, 'kova.config.json'), 'utf-8'),
    );
...
describe('CLI doctor', () => {
  ...
  it('doctor formatted output contains summary', async () => {
    ...
    expect(r.exitCode).toBe(0);
    const plain = stripAnsi(r.stdout);
    expect(plain).toContain('Kova Doctor');
