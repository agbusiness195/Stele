...
- Install `@grith/sdk` (single dependency, zero native modules)
- Generate an Ed25519 key pair
- Write CCL constraints for your agent's permitted actions
- Build and sign a covenant with `GrithClient.createCovenant()`
- Evaluate actions at runtime with `GrithClient.evaluateAction()`
...
- Use `grithMiddleware()` to enforce covenants on Express/Fastify routes
...
- Enable `keyRotation` on `GrithClient` for automatic key lifecycle management
...
- Anchor covenant hashes on-chain via `@grith/evm` for tamper-evident timestamping
...
There is no direct competitor to Grith. The concept of cryptographically signed, pre-operative constraint commitments for AI agents does not exist as a shipping product or open standard. This is a new category.
...
### Adjacent solutions and how Grith differs

| Approach | What it does | How Grith differs |
|---|---|---|
| **Guardrails (Guardrails AI, NeMo)** | Runtime input/output filtering on LLM calls | Grith governs actions, not text. Constraints are signed commitments, not runtime filters. A guardrail can be silently disabled; a covenant cannot be silently modified. |
| **RBAC / ACL systems** | Post-hoc policy enforcement by an authority | The authority can change rules at any time without the subject's knowledge. Grith's covenants are pre-operative commitments: the issuer signs before execution, and modification requires a new signature. |
| **Sandboxing (gVisor, Firecracker)** | OS-level isolation of processes | Sandboxing restricts what a process *can* do at the kernel level. Grith restricts what an agent *is committed* to doing at the application level. They are complementary: a sandbox enforces; a covenant commits. |
| **Policy engines (OPA, Cedar)** | Centralized policy evaluation | OPA evaluates policies but does not sign them. There is no cryptographic binding between the policy author and the policy content. Grith adds non-repudiation and content-addressing on top of policy evaluation. |
...
Grith answers all four. The covenant is the commitment; the signature is the proof; the CCL document is the constraint.
...
app.use(grithMiddleware({ covenant, client }));
...
```typescript
import { withGrith, withGrithTools } from '@grith/sdk';

// Wrap a single tool
const guardedTool = withGrith(myTool, { covenant, client });

// Or wrap all tools in a tool set
const guardedTools = withGrithTools(toolSet, { covenant, client });
```

### LangChain

```typescript
import { GrithCallbackHandler, withGrithTool } from '@grith/sdk';
...
const guardedTool = withGrithTool(myTool, { covenant, client });
```
...
The `@grith/mcp-server` package exposes Grith operations as MCP tools, allowing AI agents to create, verify, and evaluate covenants through the Model Context Protocol.
...
| Package | Purpose |
|---|---|
| `@grith/crypto` | Ed25519 key generation, signing, verification, hashing |
| `@grith/ccl` | Constraint Commitment Language parser and evaluator |
| `@grith/core` | Covenant build, verify, countersign, chain operations |
| `@grith/sdk` | High-level GrithClient unifying all operations |
| `@grith/store` | MemoryStore, FileStore, SqliteStore |
| `@grith/identity` | Agent identity creation, evolution, verification |
| `@grith/verifier` | Standalone verification engine with history and batch support |
| `@grith/enforcement` | Monitor (runtime enforcement), CapabilityGate, audit logs |
...
| Package | Purpose |
|---|---|
| `@grith/react` | React hooks and components for covenant UI |
| `@grith/evm` | Ethereum/EVM covenant anchoring |
| `@grith/mcp-server` | Model Context Protocol server for AI agent access |
| Express middleware | `grithMiddleware()` in `@grith/sdk` adapters |
| Vercel AI adapter | `withGrith()` / `withGrithTools()` in `@grith/sdk` adapters |
| LangChain adapter | `GrithCallbackHandler` / `withGrithTool()` in `@grith/sdk` adapters |
...
| Package | Purpose |
|---|---|
| `@grith/gametheory` | Game-theoretic analysis of covenant incentives |
| `@grith/alignment` | AI alignment verification primitives |
| `@grith/antifragile` | Antifragility patterns for covenant systems |
| `@grith/negotiation` | Multi-party covenant negotiation protocol |
| `@grith/consensus` | Distributed covenant consensus |
| `@grith/robustness` | Fault tolerance and degradation patterns |
| `@grith/temporal` | Time-varying constraint semantics |
| `@grith/recursive` | Self-referential covenant structures |
| `@grith/composition` | Covenant composition algebra |
| `@grith/attestation` | External attestation integration |
| `@grith/norms` | Social norm modeling |
| `@grith/substrate` | Cross-chain substrate layer |
| `@grith/derivatives` | Covenant derivatives and hedging |
| `@grith/legal` | Legal document mapping |
| `@grith/canary` | Canary/warrant canary primitives |
| `@grith/reputation` | Reputation scoring for agents |
| `@grith/breach` | Breach detection and response |

## 6. Roadmap Priorities

### Near-term (next release)

1. **npm publish**: Publish all stable-tier packages to npm under the `@grith` scope.
2. **Security audit**: Engage an external firm to audit `@grith/crypto`, `@grith/ccl`, and `@grith/core`. The threat model (THREAT_MODEL.md) defines the scope.
