...
import { poseidonHash, hashToField, fieldToHex } from '@kervyx/proof';
...
export async function createIdentity(
  ...
  if (!options || typeof options !== 'object') {
    throw new KervyxError(
      KervyxErrorCode.IDENTITY_INVALID,
  ...
  if (!operatorKeyPair || !operatorKeyPair.privateKey || !operatorKeyPair.publicKey || !operatorKeyPair.publicKeyHex) {
    throw new KervyxError(
      ...
      { hint: 'Generate a key pair with generateKeyPair() from @kervyx/crypto.' }
    );
  }
