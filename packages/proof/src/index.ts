...
const GROTH16_DOMAIN = {
  ...
  ALPHA_BETA: poseidonHash([BigInt('0x' + sha256String('nobulex:groth16:alpha_beta').slice(0, 60)) % FIELD_PRIME]),
  /** Domain separator for the gamma term (public input accumulation) */
  GAMMA: poseidonHash([BigInt('0x' + sha256String('nobulex:groth16:gamma').slice(0, 60)) % FIELD_PRIME]),
  /** Domain separator for the delta term (proof element C) */
  DELTA: poseidonHash([BigInt('0x' + sha256String('nobulex:groth16:delta').slice(0, 60)) % FIELD_PRIME]),
  /** Domain separator for blinding factor generation */
  BLINDING: poseidonHash([BigInt('0x' + sha256String('nobulex:groth16:blinding').slice(0, 60)) % FIELD_PRIME]),
  /** Domain separator for verification key derivation */
  VK: poseidonHash([BigInt('0x' + sha256String('nobulex:groth16:vk').slice(0, 60)) % FIELD_PRIME]),
} as const;
