import { sha256Object, signString, toHex, verify, fromHex } from '@kervyx/crypto';
...
export function createAttestation(
  ...
  if (!agentId || typeof agentId !== 'string') {
    throw new KervyxError(KervyxErrorCode.ATTESTATION_INVALID, 'agentId must be a non-empty string', { hint: 'Provide a non-empty agentId when creating an attestation.' });
  }
  if (!counterpartyId || typeof counterpartyId !== 'string') {
    throw new KervyxError(KervyxErrorCode.ATTESTATION_INVALID, 'counterpartyId must be a non-empty string', { hint: 'Provide a non-empty counterpartyId when creating an attestation.' });
  }
  if (!endpoint || typeof endpoint !== 'string') {
    throw new KervyxError(KervyxErrorCode.ATTESTATION_INVALID, 'endpoint must be a non-empty string', { hint: 'Provide a non-empty endpoint string when creating an attestation.' });
  }
  if (typeof timestamp !== 'number' || timestamp < 0) {
    throw new KervyxError(KervyxErrorCode.ATTESTATION_INVALID, 'timestamp must be a non-negative number', { hint: 'Provide a non-negative numeric timestamp when creating an attestation.' });
  }
...
export function computeAttestationCoverage(
  ...
  if (timeWindowMs < 0) {
    throw new KervyxError(KervyxErrorCode.ATTESTATION_INVALID, 'timeWindowMs must be non-negative', { hint: 'Provide a non-negative value for timeWindowMs.' });
  }
...
export function createEntanglement(params: {
  ...
  if (!params.sourceAgentId || typeof params.sourceAgentId !== 'string') {
    throw new KervyxError(KervyxErrorCode.ATTESTATION_INVALID, 'sourceAgentId must be a non-empty string', { hint: 'Provide a non-empty sourceAgentId when creating an entanglement link.' });
  }
  if (!params.targetAgentId || typeof params.targetAgentId !== 'string') {
    throw new KervyxError(KervyxErrorCode.ATTESTATION_INVALID, 'targetAgentId must be a non-empty string', { hint: 'Provide a non-empty targetAgentId when creating an entanglement link.' });
  }
  if (typeof params.strength !== 'number' || params.strength < 0 || params.strength > 1) {
    throw new KervyxError(KervyxErrorCode.ATTESTATION_INVALID, 'strength must be a number between 0 and 1', { hint: 'Set the entanglement strength to a number in the range [0, 1].' });
  }
