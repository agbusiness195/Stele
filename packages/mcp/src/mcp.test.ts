...
import { KovaGuard, PRESETS } from './index';
import type {
  MCPServer,
  WrappedMCPServer,
  ViolationDetails,
...
async function wrapEnforce(
  server?: MCPServer,
  overrides?: Partial<Parameters<typeof KovaGuard.wrap>[1]>,
): Promise<WrappedMCPServer> {
  return KovaGuard.wrap(server ?? createMockServer(), {
...
// ---------------------------------------------------------------------------
// KovaGuard.wrap basics
// ---------------------------------------------------------------------------
describe('KovaGuard.wrap', () => {
  ...
  it('wrapped server has all Kova accessors', async () => {
    const wrapped = await wrapEnforce();
...
describe('handleToolCall - permitted actions', () => {
  ...
  it('works when the original server has no handleToolCall', async () => {
    ...
    const wrapped = await KovaGuard.wrap(server, {
...
describe('PRESETS', () => {
  ...
  it('resolves a preset name via KovaGuard.wrap without error', async () => {
    const server = createMockServer();
    const wrapped = await KovaGuard.wrap(server, {
  ...
  it('passes raw CCL through when no preset matches', async () => {
    ...
    const server = createMockServer();
    const wrapped = await KovaGuard.wrap(server, { constraints: rawCCL });
...
// ---------------------------------------------------------------------------
// KovaGuard.fromCovenant
// ---------------------------------------------------------------------------
describe('KovaGuard.fromCovenant', () => {
  it('wraps a server using a pre-built covenant', async () => {
    ...
    const server = createMockServer();
    const wrapped = await KovaGuard.fromCovenant(server, covenant, kp);
  ...
  it('uses the same covenant that was passed in', async () => {
    ...
    const wrapped = await KovaGuard.fromCovenant(
      createMockServer(),
      covenant,
      kp,
    );
...
describe('edge cases', () => {
  ...
  it('throws on invalid CCL constraints', async () => {
    const server = createMockServer();
    await expect(
      KovaGuard.wrap(server, { constraints: 'this is not valid CCL %%%' }),
    ).rejects.toThrow();
  });
...
// ─── Extended KovaGuard tests ──────────────────────────────────────────────

describe('KovaGuard - extended preset tests', () => {
...
describe('KovaGuard - custom constraints', () => {
...
describe('KovaGuard - tool call interception', () => {
...
describe('KovaGuard - edge cases', () => {
  it('handles server with no tools', async () => {
    ...
    const wrapped = await KovaGuard.wrap(emptyServer, {
      operatorKeyPair: kp,
  ...
  it('handles server with many tools', async () => {
    ...
    const wrapped = await KovaGuard.wrap(manyToolsServer, {
      operatorKeyPair: kp,
