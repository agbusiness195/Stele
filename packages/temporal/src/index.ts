...
export class DecayModel {
  ...
  constructor(models: DecayModelConfig[]) {
    if (models.length === 0) {
      throw new NobulexError('At least one decay model is required', NobulexErrorCode.PROTOCOL_INVALID_INPUT);
    }
  ...
  private validateModel(model: DecayModelConfig): void {
    if (model.type === 'exponential' && model.rate < 0) {
      throw new NobulexError('Exponential decay rate must be >= 0', NobulexErrorCode.PROTOCOL_INVALID_INPUT);
    }
    if (model.type === 'linear' && model.rate < 0) {
      throw new NobulexError('Linear decay rate must be >= 0', NobulexErrorCode.PROTOCOL_INVALID_INPUT);
    }
    if (model.type === 'step') {
      if (!model.steps || model.steps.length === 0) {
        throw new NobulexError('Step decay requires at least one breakpoint', NobulexErrorCode.PROTOCOL_INVALID_INPUT);
      }
      for (const [t, v] of model.steps) {
        if (t < 0 || t > 1) {
          throw new NobulexError(`Step time fraction must be in [0, 1], got ${t}`, NobulexErrorCode.PROTOCOL_INVALID_INPUT);
        }
        if (v < 0) {
          throw new NobulexError(`Step value must be >= 0, got ${v}`, NobulexErrorCode.PROTOCOL_INVALID_INPUT);
        }
      }
    }
    if (model.type === 'seasonal') {
      if (model.rate <= 0) {
        throw new NobulexError('Seasonal frequency (rate) must be > 0', NobulexErrorCode.PROTOCOL_INVALID_INPUT);
      }
      if (model.amplitude !== undefined && (model.amplitude < 0 || model.amplitude > 1)) {
        throw new NobulexError(`Seasonal amplitude must be in [0, 1], got ${model.amplitude}`, NobulexErrorCode.PROTOCOL_INVALID_INPUT);
      }
    }
  }
  ...
  private evaluateOne(model: DecayModelConfig, t: number): number {
    switch (model.type) {
      ...
      default:
        throw new NobulexError(`Unknown decay model type: ${model.type}`, NobulexErrorCode.PROTOCOL_COMPUTATION_FAILED);
    }
  }
  ...
  evaluate(t: number, initialWeight: number = 1.0): number {
    if (initialWeight < 0) {
      throw new NobulexError('initialWeight must be >= 0', NobulexErrorCode.PROTOCOL_INVALID_INPUT);
    }
  ...
  schedule(initialWeight: number, steps: number): DecayPoint[] {
    if (steps < 2) {
      throw new NobulexError('steps must be >= 2', NobulexErrorCode.PROTOCOL_INVALID_INPUT);
    }
...
export class ContinuousTrigger {
  ...
  constructor(triggers: ContinuousTriggerConfig[], activationThreshold: number = 0.5) {
    if (triggers.length === 0) {
      throw new NobulexError('At least one trigger is required', NobulexErrorCode.PROTOCOL_INVALID_INPUT);
    }
    if (activationThreshold < 0 || activationThreshold > 1) {
      throw new NobulexError('activationThreshold must be in [0, 1]', NobulexErrorCode.PROTOCOL_INVALID_INPUT);
    }
    for (const t of triggers) {
      if (t.steepness !== undefined && t.steepness <= 0) {
        throw new NobulexError('steepness must be > 0', NobulexErrorCode.PROTOCOL_INVALID_INPUT);
      }
      if (t.weight !== undefined && t.weight < 0) {
        throw new NobulexError('weight must be >= 0', NobulexErrorCode.PROTOCOL_INVALID_INPUT);
      }
    }
...
export class ViolationForecaster {
  ...
  constructor(config: ForecastConfig) {
    if (config.alpha <= 0 || config.alpha >= 1) {
      throw new NobulexError('alpha must be in (0, 1)', NobulexErrorCode.PROTOCOL_INVALID_INPUT);
    }
    if (config.beta <= 0 || config.beta >= 1) {
      throw new NobulexError('beta must be in (0, 1)', NobulexErrorCode.PROTOCOL_INVALID_INPUT);
    }
    if (config.forecastPeriods < 1) {
      throw new NobulexError('forecastPeriods must be >= 1', NobulexErrorCode.PROTOCOL_INVALID_INPUT);
    }
    if (config.confidenceLevel !== undefined) {
      if (config.confidenceLevel <= 0 || config.confidenceLevel >= 1) {
        throw new NobulexError('confidenceLevel must be in (0, 1)', NobulexErrorCode.PROTOCOL_INVALID_INPUT);
      }
    }
    this.config = config;
  }
  ...
  forecast(historicalRates: number[]): ForecastResult {
    if (historicalRates.length < 2) {
      throw new NobulexError('At least 2 historical data points required', NobulexErrorCode.PROTOCOL_INVALID_INPUT);
    }

    for (let i = 0; i < historicalRates.length; i++) {
      if (typeof historicalRates[i] !== 'number' || isNaN(historicalRates[i]!)) {
        throw new NobulexError(`historicalRates[${i}] must be a valid number`, NobulexErrorCode.PROTOCOL_INVALID_INPUT);
      }
    }
...
export class TemporalConstraintAlgebra {
  ...
  private validate(constraint: TemporalConstraint): void {
    if (constraint.start < 0 || constraint.start > 1) {
      throw new NobulexError(`start must be in [0, 1], got ${constraint.start}`, NobulexErrorCode.PROTOCOL_INVALID_INPUT);
    }
    if (constraint.end < 0 || constraint.end > 1) {
      throw new NobulexError(`end must be in [0, 1], got ${constraint.end}`, NobulexErrorCode.PROTOCOL_INVALID_INPUT);
    }
    if (constraint.start > constraint.end) {
      throw new NobulexError(`start (${constraint.start}) must be <= end (${constraint.end})`, NobulexErrorCode.PROTOCOL_INVALID_INPUT);
    }
    if (constraint.weight < 0 || constraint.weight > 1) {
      throw new NobulexError(`weight must be in [0, 1], got ${constraint.weight}`, NobulexErrorCode.PROTOCOL_INVALID_INPUT);
    }
  }
