...
export async function generateKeyPair(): Promise<KeyPair> {
  ...
  if (privateKey.length !== 32) {
    throw new KervyxError(
      KervyxErrorCode.CRYPTO_INVALID_KEY,
...
export async function sign(message: Uint8Array, privateKey: PrivateKey): Promise<Signature> {
  if (!(message instanceof Uint8Array)) {
    throw new KervyxError(
      KervyxErrorCode.CRYPTO_SIGNATURE_FAILED,
...
export function base64urlDecode(encoded: Base64Url): Uint8Array {
  if (typeof encoded !== 'string') {
    throw new KervyxError(
      KervyxErrorCode.CRYPTO_INVALID_HEX,
 ...
 *
 * Uses the current system time. All Kervyx protocol timestamps are UTC.
 *
