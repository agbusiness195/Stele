...
Ed25519 cryptography, SHA-256 hashing, and encoding utilities for the Grith protocol. Built on [@noble/ed25519](https://github.com/paulmillr/noble-ed25519) and [@noble/hashes](https://github.com/paulmillr/noble-hashes).

## Installation

```bash
npm install @grith/crypto
```
...
```typescript
import { generateKeyPair, sign, verify, toHex } from '@grith/crypto';
...
```typescript
import { sha256, sha256String, sha256Object } from '@grith/crypto';
...
```typescript
import { canonicalizeJson } from '@grith/crypto';
...
```typescript
import { toHex, fromHex, base64urlEncode, base64urlDecode } from '@grith/crypto';
...
```typescript
import { KeyManager } from '@grith/crypto';
