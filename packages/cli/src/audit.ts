...
export function runAudit(params: {
  ...
  if (covenantCount === 0) {
    findings.push({
      ...
      recommendation: 'Create at least one covenant with appropriate constraints using "kervyx create".',
      autoFixable: false,
    });
  ...
  if (!identityVerified) {
    findings.push({
      ...
      recommendation: 'Complete identity verification using "kervyx init" and register with a discovery server.',
      autoFixable: false,
    });
