/**
 * @grith/cli configuration file support.
 *
 * Reads and writes `grith.config.json` in the current working directory.
...
/** Shape of a `grith.config.json` configuration file. */
export interface GrithConfig {
...
export const CONFIG_FILE_NAME = 'grith.config.json';

// ─── Public API ───────────────────────────────────────────────────────────────

/**
 * Search for a `grith.config.json` starting from `cwd` and walking up to the
...
/**
 * Load the `grith.config.json` starting from `cwd`.
 * Returns `undefined` if no config file is found.
 * Throws if the file exists but cannot be parsed.
 */
export function loadConfig(cwd?: string): GrithConfig | undefined {
  const filePath = findConfigFile(cwd);
  if (!filePath) return undefined;

  const raw = readFileSync(filePath, 'utf-8');
  const parsed = JSON.parse(raw) as GrithConfig;
  return parsed;
}

/**
 * Write a `grith.config.json` to the given directory (defaults to cwd).
 * Overwrites any existing config file at that location.
 */
export function saveConfig(config: GrithConfig, cwd?: string): void {
