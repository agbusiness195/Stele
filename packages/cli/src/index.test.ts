...
import { buildCovenant, serializeCovenant, PROTOCOL_VERSION } from '@grith/core';
...
describe('grith help', () => {
  it('shows help with no arguments', async () => {
    ...
    expect(r.stdout).toContain('Grith CLI');
...
describe('grith init', () => {
  it('generates a key pair and prints public key', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'grith-test-'));
    try {
  ...
  it('writes grith.config.json on init', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'grith-test-'));
    try {
      const r = await run(['init'], tmp);
      expect(r.exitCode).toBe(0);
      const configPath = join(tmp, 'grith.config.json');
...
describe('grith verify', () => {
  ...
  it('shows help with --help', async () => {
    ...
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain('grith verify');
  });
...
describe('grith evaluate', () => {
  ...
  it('shows help with --help', async () => {
    ...
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain('grith evaluate');
  });
});
...
describe('grith inspect', () => {
  ...
  it('shows help with --help', async () => {
    ...
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain('grith inspect');
  });
});
...
describe('grith parse', () => {
  ...
  it('shows help with --help', async () => {
    ...
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain('grith parse');
  });
});
...
describe('grith completions', () => {
  ...
  it('generates zsh completion script', async () => {
    ...
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain('#compdef grith');
    expect(r.stdout).toContain('_grith');
  ...
  it('generates fish completion script', async () => {
    ...
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain('complete -c grith');
    expect(r.stdout).toContain('init');
    expect(r.stdout).toContain('create');
    expect(r.stdout).toContain('doctor');
    expect(r.stdout).toContain('diff');
  ...
  it('shows help with --help', async () => {
    ...
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain('grith completions');
...
describe('unknown command', () => {
  it('returns error for unknown command', async () => {
    ...
    expect(r.stderr).toContain('grith help');
  });
});
...
describe('config loading', () => {
  it('loads config from specified directory', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'grith-cfg-'));
    try {
      // Write a config file
  ...
  it('works gracefully when no config file exists', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'grith-nocfg-'));
    try {
...
describe('grith doctor', () => {
  it('returns checks with colored output', async () => {
    const r = await run(['doctor']);
    expect(r.exitCode).toBe(0);
    expect(stripAnsi(r.stdout)).toContain('Grith Doctor');
  ...
  it('shows help with --help', async () => {
    ...
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain('grith doctor');
...
describe('grith diff', () => {
  ...
  it('shows help with --help', async () => {
    ...
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain('grith diff');
