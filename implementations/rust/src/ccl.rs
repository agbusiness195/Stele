...
//! in Kova covenants. It supports four statement types:
//!
...
impl Parser {
    ...
    fn expect(&mut self, tt: &TokenType, msg: &str) -> Result<Token, KovaError> {
        if self.current().token_type == *tt {
            Ok(self.advance())
        } else {
            Err(KovaError::CCLParseError(format!(
    ...
    fn parse(&mut self) -> Result<CCLDocument, KovaError> {
    ...
    fn parse_statement(&mut self) -> Result<Statement, KovaError> {
        match self.current().token_type {
            ...
            _ => Err(KovaError::CCLParseError(format!(
    ...
    fn parse_permit_deny(&mut self) -> Result<Statement, KovaError> {
    ...
    fn parse_require(&mut self) -> Result<Statement, KovaError> {
    ...
    fn parse_limit(&mut self) -> Result<Statement, KovaError> {
        ...
        let count: f64 = count_tok
            .value
            .parse()
            .map_err(|_| KovaError::CCLParseError(format!("Invalid count number: {}", count_tok.value)))?;
        ...
        let raw_period: f64 = period_tok
            .value
            .parse()
            .map_err(|_| KovaError::CCLParseError(format!("Invalid period number: {}", period_tok.value)))?;
    ...
    fn parse_action(&mut self) -> Result<String, KovaError> {
        ...
        if self.check(&TokenType::Wildcard) {
            ...
            return Err(KovaError::CCLParseError(format!(
    ...
    fn parse_resource(&mut self) -> Result<String, KovaError> {
        match self.current().token_type {
    ...
    fn parse_condition(&mut self) -> Result<Condition, KovaError> {
        // Parse the field
    ...
    fn parse_field(&mut self) -> Result<String, KovaError> {
    ...
    fn parse_value(&mut self) -> Result<String, KovaError> {
        match self.current().token_type {
            TokenType::StringLit => Ok(self.advance().value),
...
///
/// # Errors
/// Returns `KovaError::CCLParseError` if the source contains syntax errors.
///
/// # Example
/// ```
/// use kova::ccl::parse;
...
/// ```
pub fn parse(source: &str) -> Result<CCLDocument, KovaError> {
