...
pub fn canonical_form(doc: &CovenantDocument) -> Result<String, KovaError> {
    // Build a JSON value, then remove the mutable fields
    let val = serde_json::to_value(doc)
        .map_err(|e| KovaError::SerializationError(format!("Failed to convert to JSON value: {}", e)))?;
...
pub fn compute_id(doc: &CovenantDocument) -> Result<String, KovaError> {
...
///
/// # Errors
/// Returns `KovaError::InvalidInput` for missing/invalid fields,
/// `KovaError::CCLParseError` for invalid constraints, or
/// `KovaError::CryptoError` for signing failures.
pub fn build_covenant(opts: CovenantBuilderOptions) -> Result<CovenantDocument, KovaError> {
    // Validate required inputs
    if opts.issuer.id.is_empty() {
        return Err(KovaError::InvalidInput("issuer.id is required".to_string()));
    }
    ...
    if parsed_ccl.statements.len() > MAX_CONSTRAINTS {
        return Err(KovaError::InvalidInput(format!(
    ...
    let serialized = serde_json::to_string(&doc)
        .map_err(|e| KovaError::SerializationError(format!("Failed to serialize: {}", e)))?;
...
pub fn verify_covenant(doc: &CovenantDocument) -> Result<VerificationResult, KovaError> {
...
pub fn countersign_covenant(
    ...
    kp: &crypto::KeyPair,
    role: &str,
) -> Result<CovenantDocument, KovaError> {
    let canonical = canonical_form(doc)?;
...
pub fn serialize_covenant(doc: &CovenantDocument) -> Result<String, KovaError> {
    serde_json::to_string_pretty(doc)
        .map_err(|e| KovaError::SerializationError(format!("Failed to serialize covenant: {}", e)))
}

/// Deserialize a JSON string into a CovenantDocument.
pub fn deserialize_covenant(json: &str) -> Result<CovenantDocument, KovaError> {
    serde_json::from_str(json)
        .map_err(|e| KovaError::SerializationError(format!("Failed to deserialize covenant: {}", e)))
}
...
) -> Result<ccl::NarrowingResult, KovaError> {
