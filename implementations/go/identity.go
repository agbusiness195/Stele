...
func CreateIdentity(opts *CreateIdentityOptions) (*AgentIdentity, error) {
	if opts == nil {
		return nil, fmt.Errorf("nobulex: createIdentity requires options")
	}
	if opts.OperatorKeyPair == nil {
		return nil, fmt.Errorf("nobulex: operatorKeyPair is required")
	}
	if opts.Model.Provider == "" || opts.Model.ModelID == "" {
		return nil, fmt.Errorf("nobulex: model.provider and model.modelId are required")
	}
	if opts.Capabilities == nil {
		return nil, fmt.Errorf("nobulex: capabilities array is required")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("nobulex: failed to compute identity hash: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("nobulex: failed to compute lineage signing payload: %w", err)
	}
	lineageSig, err := Sign([]byte(lineagePayload), opts.OperatorKeyPair.PrivateKey)
	if err != nil {
		return nil, fmt.Errorf("nobulex: failed to sign lineage entry: %w", err)
	}
	...
	idHash, err = computeIdentityHash(identity)
	if err != nil {
		return nil, fmt.Errorf("nobulex: failed to recompute identity hash: %w", err)
	}
	identity.ID = idHash
	...
	if err != nil {
		return nil, fmt.Errorf("nobulex: failed to compute identity signing payload: %w", err)
	}
	sig, err := Sign([]byte(payload), opts.OperatorKeyPair.PrivateKey)
	if err != nil {
		return nil, fmt.Errorf("nobulex: failed to sign identity: %w", err)
	}
...
func EvolveIdentity(current *AgentIdentity, opts *EvolveIdentityOptions) (*AgentIdentity, error) {
	if current == nil {
		return nil, fmt.Errorf("nobulex: current identity is required")
	}
	if opts == nil {
		return nil, fmt.Errorf("nobulex: evolve options are required")
	}
	if opts.OperatorKeyPair == nil {
		return nil, fmt.Errorf("nobulex: operatorKeyPair is required")
	}
	if opts.ChangeType == "" {
		return nil, fmt.Errorf("nobulex: changeType is required")
	}
	if opts.Description == "" {
		return nil, fmt.Errorf("nobulex: description is required")
	}
...
func VerifyIdentity(identity *AgentIdentity) (bool, error) {
	if identity == nil {
		return false, fmt.Errorf("nobulex: identity is required")
	}

	// Verify the identity signature
	payload, err := identitySigningPayload(identity)
	if err != nil {
		return false, fmt.Errorf("nobulex: failed to compute signing payload: %w", err)
	}
