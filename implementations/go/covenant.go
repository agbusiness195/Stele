...
func CanonicalForm(doc *CovenantDocument) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("nobulex: failed to convert document to map: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("nobulex: failed to canonicalize document: %w", err)
	}
...
func BuildCovenant(opts *CovenantBuilderOptions) (*CovenantDocument, error) {
	...
	if opts.Issuer.ID == "" {
		return nil, fmt.Errorf("nobulex: issuer.id is required")
	}
	if opts.Issuer.PublicKey == "" {
		return nil, fmt.Errorf("nobulex: issuer.publicKey is required")
	}
	if opts.Issuer.Role != "issuer" {
		return nil, fmt.Errorf("nobulex: issuer.role must be 'issuer'")
	}
	if opts.Beneficiary.ID == "" {
		return nil, fmt.Errorf("nobulex: beneficiary.id is required")
	}
	if opts.Beneficiary.PublicKey == "" {
		return nil, fmt.Errorf("nobulex: beneficiary.publicKey is required")
	}
	if opts.Beneficiary.Role != "beneficiary" {
		return nil, fmt.Errorf("nobulex: beneficiary.role must be 'beneficiary'")
	}
	if strings.TrimSpace(opts.Constraints) == "" {
		return nil, fmt.Errorf("nobulex: constraints is required")
	}
	if len(opts.PrivateKey) != ed25519.PrivateKeySize {
		return nil, fmt.Errorf("nobulex: privateKey must be %d bytes", ed25519.PrivateKeySize)
	}

	// Parse CCL to verify syntax and check constraint count
	parsedCCL, err := Parse(opts.Constraints)
	if err != nil {
		return nil, fmt.Errorf("nobulex: invalid CCL constraints: %w", err)
	}
	if len(parsedCCL.Statements) > MaxConstraints {
		return nil, fmt.Errorf("nobulex: constraints exceed maximum of %d statements (got %d)", MaxConstraints, len(parsedCCL.Statements))
	}

	// Validate chain reference
	if opts.Chain != nil {
		if opts.Chain.ParentID == "" {
			return nil, fmt.Errorf("nobulex: chain.parentId is required")
		}
		if opts.Chain.Relation == "" {
			return nil, fmt.Errorf("nobulex: chain.relation is required")
		}
		if opts.Chain.Depth < 1 {
			return nil, fmt.Errorf("nobulex: chain.depth must be a positive integer")
		}
		if opts.Chain.Depth > MaxChainDepth {
			return nil, fmt.Errorf("nobulex: chain.depth exceeds maximum of %d (got %d)", MaxChainDepth, opts.Chain.Depth)
		}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("nobulex: failed to sign covenant: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("nobulex: failed to serialize covenant: %w", err)
	}
	if len(serialized) > MaxDocumentSize {
		return nil, fmt.Errorf("nobulex: serialized document exceeds maximum size of %d bytes", MaxDocumentSize)
	}
...
func CountersignCovenant(doc *CovenantDocument, kp *KeyPair, role string) (*CovenantDocument, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("nobulex: failed to countersign: %w", err)
	}
...
func SerializeCovenant(doc *CovenantDocument) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("nobulex: failed to serialize covenant: %w", err)
	}
	return string(b), nil
}
...
func DeserializeCovenant(jsonStr string) (*CovenantDocument, error) {
	...
	if err := json.Unmarshal([]byte(jsonStr), &doc); err != nil {
		return nil, fmt.Errorf("nobulex: invalid JSON: %w", err)
	}

	// Validate required fields
	if doc.ID == "" {
		return nil, fmt.Errorf("nobulex: missing required field: id")
	}
	if doc.Version == "" {
		return nil, fmt.Errorf("nobulex: missing required field: version")
	}
	if doc.Version != ProtocolVersion {
		return nil, fmt.Errorf("nobulex: unsupported protocol version: %s (expected %s)", doc.Version, ProtocolVersion)
	}
	if doc.Issuer.ID == "" || doc.Issuer.PublicKey == "" || doc.Issuer.Role != "issuer" {
		return nil, fmt.Errorf("nobulex: invalid issuer: must have id, publicKey, and role='issuer'")
	}
	if doc.Beneficiary.ID == "" || doc.Beneficiary.PublicKey == "" || doc.Beneficiary.Role != "beneficiary" {
		return nil, fmt.Errorf("nobulex: invalid beneficiary: must have id, publicKey, and role='beneficiary'")
	}
	if doc.Constraints == "" {
		return nil, fmt.Errorf("nobulex: missing required field: constraints")
	}
	if doc.Nonce == "" {
		return nil, fmt.Errorf("nobulex: missing required field: nonce")
	}
	if doc.CreatedAt == "" {
		return nil, fmt.Errorf("nobulex: missing required field: createdAt")
	}
	if doc.Signature == "" {
		return nil, fmt.Errorf("nobulex: missing required field: signature")
	}

	// Validate chain if present
	if doc.Chain != nil {
		if doc.Chain.ParentID == "" {
			return nil, fmt.Errorf("nobulex: invalid chain.parentId: must be a string")
		}
		if doc.Chain.Relation == "" {
			return nil, fmt.Errorf("nobulex: invalid chain.relation: must be a string")
		}
	}

	// Validate document size
	if len(jsonStr) > MaxDocumentSize {
		return nil, fmt.Errorf("nobulex: document size %d bytes exceeds maximum of %d bytes", len(jsonStr), MaxDocumentSize)
	}
...
func ValidateChainNarrowing(child, parent *CovenantDocument) (*NarrowingResult, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("nobulex: failed to parse parent constraints: %w", err)
	}
	childCCL, err := Parse(child.Constraints)
	if err != nil {
		return nil, fmt.Errorf("nobulex: failed to parse child constraints: %w", err)
	}
