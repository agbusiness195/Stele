...
func GenerateKeyPair() (*KeyPair, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("kova: failed to generate Ed25519 key pair: %w", err)
	}
	return &KeyPair{
		PrivateKey:   priv,
		PublicKey:     pub,
...
func KeyPairFromPrivateKey(privateKey ed25519.PrivateKey) (*KeyPair, error) {
	if len(privateKey) != ed25519.PrivateKeySize {
		return nil, fmt.Errorf("kova: private key must be %d bytes, got %d", ed25519.PrivateKeySize, len(privateKey))
	}
...
func CanonicalizeJSON(obj interface{}) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("kova: failed to marshal canonical JSON: %w", err)
	}
	return string(b), nil
}
...
func FromHex(hexStr string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("kova: invalid hex string: %w", err)
	}
	return b, nil
}
...
func GenerateNonce() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("kova: failed to generate nonce: %w", err)
	}
	return nonce, nil
}
