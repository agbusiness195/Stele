...
for the Kervyx protocol. It extends the existing `.well-known/kervyx/` discovery
...
4. **Incremental adoption.** The mechanism must be backward-compatible with
   the existing `.well-known/kervyx/` protocol.
...
```json
{
  ...
  "endpoints": [
    "https://agent-a.example.com/.well-known/kervyx/",
    "ipfs://Qm.../kervyx/"
  ],
...
- Agents that only support `.well-known/kervyx/` continue to work unchanged.
