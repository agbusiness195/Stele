...
for the Stele protocol. It extends the existing `.well-known/stele/` discovery
...
4. **Incremental adoption.** The mechanism must be backward-compatible with
   the existing `.well-known/stele/` protocol.
...
```json
{
  ...
  "endpoints": [
    "https://agent-a.example.com/.well-known/stele/",
    "ipfs://Qm.../stele/"
  ],
...
- Agents that only support `.well-known/stele/` continue to work unchanged.
